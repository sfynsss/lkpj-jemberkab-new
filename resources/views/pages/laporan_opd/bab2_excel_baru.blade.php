<table style="border: 1px; border-color: black;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td colspan="11" class="text-left">{{ $urusan->nama_urusan }}</td>
    </tr>
    
    @foreach($urusan->BidangUrusan as $bidang)
    @if(isset($bidang->Program[0]) && $bidang->Program[0]->id_skpd == $skpd->id)
    <tr>
      <td class="text-left">{{ $bidang->kode_bidang_urusan }}</td>
      <td colspan="11" class="text-left">{{ $bidang->nama_bidang_urusan }}</td>
    </tr>
    @endif
    ...
    <tr>
      ...
      <td colspan="2" class="text-left">{{ $program->nama_program }}</td>
      <td colspan="9" class="text-left">@if ($program->id_sasaran != 0) {{ App\SasaranBaru::where('id', $program->id_sasaran)->pluck('nama_sasaran')->first() }} @endif</td>
    </tr>
