  ...
  <body>
    ...
    <table>
        <tbody>
            ...
            @foreach ($skpd as $opd) {{-- start_opd --}}
            <tr>
                <td align=center><span></span></td>
                <td align=right><span></span></td>
                <td align=left><span>{{$opd->kode_skpd}}</span></td>
                <td align=left colspan="25"><span>{{$opd->nama_skpd}}</span></td>
            </tr>
                @foreach ($urusan as $v_urus) {{-- start_urusan --}}
                <tr>
                    <td align=center><span></span></td>
                    <td align=right><span></span></td>
                    <td align=left><span>{{$v_urus->kode_urusan}}</span></td>
                    <td align=left colspan="25"><span>{{$v_urus->nama_urusan}}</span></td>
                </tr>
                    @foreach ($v_urus->BidangUrusan as $v_bidang_urusan) {{-- start_bidangurusan --}}
                    <tr>
                        <td align=center><span></span></td>
                        <td align=right><span></span></td>
                        <td align=left><span>{{$v_bidang_urusan->kode_bidang_urusan}}</span></td>
                        <td align=left colspan="25"><span>{{$v_bidang_urusan->nama_bidang_urusan}}</span></td>
                    </tr>
                        @foreach ($v_bidang_urusan->Program as $v_program) {{-- start_program --}}
                        <tr>
                            <td align=center><span></span></td>
                            <td align=left><span></span>{{ $sasaran->where('id', $v_program->id_sasaran)->pluck('kode_sasaran')->first() }}</td>
                            <td align=left><span>{{$v_program->kode_program}}</span></td>
                            <td align=left colspan="25"><span>{{$v_program->nama_program}}</span></td>
                        </tr>
                            @foreach ($v_program->Kegiatan as $v_kegiatan) {{-- start_kegiatan --}}
                            <tr>
                                <td align=center><span></span></td>
                                <td align=right><span></span></td>
                                <td align=left><span>{{$v_kegiatan->kode_kegiatan}}</span></td>
                                <td align=left colspan="25"><span>{{$v_kegiatan->nama_kegiatan}}</span></td>
                            </tr>
                                @foreach ($v_kegiatan->SubKegiatan as $v_subkeg) {{-- start_subkeg --}}
                                <tr>
                                    <td align=center><span></span></td> {{-- 1--}}
                                    <td align=right><span></span></td> {{-- 2--}}
                                    <td align=left><span>{{$v_subkeg->kode_sub_keg}}</span></td> {{-- 3--}}
                                    <td align=left><span>{{$v_subkeg->nama_sub_keg}}</span></td> {{-- 4--}}
                                    <td>{{$v_subkeg->indikator_sub_keg}}</td> {{-- 5--}}
                                    <td>-</td> {{-- 6k--}}
                                    <td>Rp. {{ number_format(($v_subkeg->pagu_indikatif_sub_keg_2 * 5) + (($v_subkeg->pagu_indikatif_sub_keg_2 * 5) * 0.05), 0,',','.') }}</td> {{-- 6r--}}
                                    <td></td> {{-- 7k--}}
                                    <td>@php $tujuh = $v_subkeg->serapan_sub_keg_1_1 + $v_subkeg->serapan_sub_keg_1_2; @endphp Rp. {{ number_format($tujuh, 0,',','.') }}</td> {{-- 7r--}}
                                    <td align=center>{{$v_subkeg->target_sub_keg_2}}</td> {{-- 8k--}}
                                    <td>Rp. {{ number_format($v_subkeg->pagu_indikatif_sub_keg_2, 0,',','.') }}</td> {{-- 8r--}}
                                    <td align=center></td> {{-- 9k--}}
                                    <td>Rp. {{ number_format($v_subkeg->serapan_sub_keg_2_1, 0,',','.') }}</td> {{-- 9r--}}
                                    <td align=center></td> {{-- 10k--}}
                                    <td>Rp. {{ number_format($v_subkeg->serapan_sub_keg_2_2, 0,',','.') }}</td> {{-- 10r--}}
                                    <td align=center></td> {{-- 11k--}}
                                    <td align=center></td> {{-- 11r--}}
                                    <td align=center></td> {{-- 12k--}}
                                    <td align=center></td> {{-- 12r--}}
                                    <td>@php $tigabelas = $v_subkeg->serapan_sub_keg_2_1 + $v_subkeg->serapan_sub_keg_2_2; @endphp Rp. {{ number_format($tigabelas, 0,',','.') }}</td> {{-- 13k--}}
                                    <td align=center></td> {{-- 13r--}}
                                    <td align=center></td> {{-- 14k--}}
                                    <td align=center></td> {{-- 14r--}}
                                    <td align=center></td> {{-- 15k--}}
                                    <td>Rp. {{ number_format($tujuh + $tigabelas, 0,',','.') }}</td> {{-- 15r--}}
                                    <td align=center></td> {{-- 16k--}}
                                    <td align=center></td> {{-- 16r--}}
                                    <td align=center></td> {{-- 17k--}}
                                    <td align=center></td> {{-- 17r--}}
                                </tr>
                                @endforeach {{-- end_subkeg --}}
                            @endforeach {{-- end_kegiatan --}}
                        @endforeach {{-- end_program --}}
                    @endforeach {{-- end_bidangurusan --}}
                @endforeach {{-- end_urusan --}}
            @endforeach {{-- end_opd --}}
        </tbody>
