...
@include('pages.opd.kinerja.kinerja_program')
            ...
            <table id="basic-datatables" class="display table table-striped table-hover" >
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="{{ route('kinerja-kegiatan-opd', $data->id) }}" class="btn btn-primary" ><i class="fas fa-clipboard-check btn-icon-prepend mr-2"></i>Kegiatan</a>
                    <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModal" onclick="kinerjaProgram(
                      '{{$data->id}}',
                      '{{$data->kode_program}}',
                      '{{$data->nama_program}}',
                      '{{$data->indikator_program}}',
                      '{{$data->pagu_indikatif_program1}}',
                      '{{$data->realisasi_program1}}',
                      '{{$data->target_program1}}',
                      '{{$data->kinerja_program1}}'
                      )"><i class="fas fa-file btn-icon-prepend mr-2"></i> Kinerja Program
                    </button>
                  </td>
                </tr>
                @endforeach
